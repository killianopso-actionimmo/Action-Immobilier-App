...
const callProxy = async (contents: any, systemInstruction: string, tools?: any[]) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error((errorData.error || "Erreur serveur") + " [V-FRONT-1.1.1]");
    }
...
export const getApiStatus = (): string => "SERVER_MODE_1.1.1_SNAKE_REST";
